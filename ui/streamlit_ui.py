...
from services.cv_evaluator import evaluador_cv
...
def procesar_analisis(archivo_cv, descripcion_puesto):
    ...
    with st.spinner("🔄 Procesando currículum..."):
        ...
        resultado = evaluador_cv(texto_cv, descripcion_puesto)
...
def mostrar_resultados(resultado: AnalisisCv):
    ...
    if resultado.porcentaje_adecuacion >= 80:
        color = "🟢"
        nivel = "EXCELENTE"
        mensaje = "Candidato altamente recomendado"
    elif resultado.porcentaje_adecuacion >= 60:
        color = "🟡"
        nivel = "BUENO"
        mensaje = "Candidato recomendado con reservas"
    elif resultado.porcentaje_adecuacion >= 40:
        color = "🟠"
        nivel = "REGULAR"
    ...
    with col2:
        st.metric(
            ...
            value=f"{resultado.porcentaje_adecuacion}%",
            delta=f"{color} {nivel}"
        )
    ...
    if resultado.habilidades_tecnicas:
        cols = st.columns(min(len(resultado.habilidades_tecnicas), 4))
        for i, habilidad in enumerate(resultado.habilidades_tecnicas):
            with cols[i % 4]:
    ...
    if resultado.porcentaje_adecuacion >= 70:
        st.success("""
        ...
        """)
    elif resultado.porcentaje_adecuacion >= 50:
        st.warning("""
